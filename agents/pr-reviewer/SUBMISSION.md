...
- Supports optional merge gating with `--fail-on Low|Medium|High`, which fails after writing the review when findings meet the configured severity threshold.
...
- Optional `--fail-on` gates run after output/comment/summary generation, so a blocking workflow still leaves reviewer-visible evidence.
- Composite action inputs are passed through environment variables and bash arrays instead of direct shell interpolation.
...
```text
Temporary fork-only PR: https://github.com/lymeow233-wq/claude-builders-bounty/pull/2
Run: https://github.com/lymeow233-wq/claude-builders-bounty/actions/runs/25775642309
Result: success
Verified: composite action installed the package with `actions/checkout@v6` and `actions/setup-python@v6`, ran `claude-review --pr ... --repo-root . --github-annotations --step-summary --update-comment`, and posted the bot comment.
```
